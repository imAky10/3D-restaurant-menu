let Menu = (function () {
	...
	var $container = $('#rm-container'),
		...
		viewDetails = function (recipe) {
			...
			let title = recipe.text(),
				img = recipe.data('thumb'),
				description = recipe.parent().next().text();


			let $modal = $('<div class="rm-modal"><div class="rm-thumb" style="background-image: url(' + img + ')"></div><h5>' + title + '</h5><p>' + description + '</p><span class="rm-close-modal">&#10006;</span></div>');

			$modal.appendTo($container);

			let h = $modal.outerHeight(true);
